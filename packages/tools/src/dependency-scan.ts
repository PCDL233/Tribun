...
const ADVISORIES: readonly Advisory[] = [
  { packageName: 'lodash', fixedVersion: [4, 17, 21], cveId: 'CVE-2021-23337', severity: 'BLOCKER', summary: 'lodash 旧版本存在命令注入风险' },
  { packageName: 'minimist', fixedVersion: [1, 2, 6], cveId: 'CVE-2021-44906', severity: 'WARNING', summary: 'minimist 旧版本存在原型污染风险' },
  { packageName: 'axios', fixedVersion: [1, 6, 0], cveId: 'CVE-2023-45857', severity: 'WARNING', summary: 'axios 旧版本存在 CSRF/凭据泄露风险' },
  { packageName: 'jsonwebtoken', fixedVersion: [9, 0, 0], cveId: 'CVE-2022-23529', severity: 'BLOCKER', summary: 'jsonwebtoken 旧版本存在远程代码执行风险' },
];
...
function isOlder(version: readonly number[], fixed: readonly number[]): boolean {
  for (let index = 0; index < fixed.length; index += 1) {
    if ((version[index] ?? 0) !== (fixed[index] ?? 0)) return (version[index] ?? 0) < (fixed[index] ?? 0);
  }
  return false;
}
...
function lineForDependency(file: FileContext, packageName: string): { line: number; snippet?: string } | null {
...
export function dependencyFindings(files: readonly FileContext[]): Finding[] {
  return files.flatMap((file) => {
    ...
    return ADVISORIES.flatMap((advisory): Finding[] => {
      ...
      if (version === null || /^[*><=]/.test(requested.trim()) || !isOlder(version, advisory.fixedVersion)) return [];
