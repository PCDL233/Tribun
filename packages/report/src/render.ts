...
export function renderHtml(report: ReviewReport): string {
  const findings = report.findings.length === 0
    ...
    : report.findings.map((finding) => `<tr><td><span class="severity severity-${finding.severity.toLowerCase()}">${escapeHtml(finding.severity)}</span></td><td><code>${escapeHtml(`${finding.filePath}:${finding.lineStart}`)}</code></td><td><strong>${escapeHtml(finding.title)}</strong><br><span class="muted">${escapeHtml(finding.description)}</span>${finding.suggestion === undefined ? '' : `<br><span class="suggestion">建议：${escapeHtml(finding.suggestion)}</span>`}</td><td>${Math.round(finding.confidence * 100)}%</td><td>${escapeHtml(finding.agent)}</td></tr>`).join('');
  const list = (items: readonly string[]): string => items.length === 0 ? '<p class="muted">暂无</p>' : `<ul>${items.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>`;
