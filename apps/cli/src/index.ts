...
program
  ...
  .action((options: { paths?: string[]; model?: string; baseUrl?: string }) => {
    const { paths, model, baseUrl } = options;
...
program.command('server').description('启动 API Server + Dashboard')
  ...
  .action((options: { port: string; db?: string; webDist?: string }) => void startServerCommand(options));
